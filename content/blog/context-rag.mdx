...
## Test the Answer, Not Retrieval

Evaluate contextual retrieval with real questions from users, then judge the answer they receive. Retrieval accuracy matters only when it improves the result.

A system can find the expected documents and still produce an answer that does not help. The final test is whether the user can complete the job they came to do.
