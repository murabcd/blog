...
## Agents Should Travel Light

A capable agent does not need every tool in every context. It needs to know what is available, then load the smallest useful set when the task demands it.

That is the point of modular skills. They reduce context cost without reducing capability. The agent can still do broad work, but it does not have to carry the entire system while doing one narrow job.
