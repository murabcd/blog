...
The product structure that helps a company find a market will eventually limit its ability to scale one. The work changes, so the ownership, skills, and communication paths have to change with it.

Do not reorganize because the company reached an arbitrary headcount. Reorganize when the current structure no longer matches the problem the business is trying to solve. A product organization should fit the company's present constraint, not preserve the shape that worked in the previous stage.
