...
The best pattern is to give the agent a "Cheat Sheet" (often called `context.md`). This is a file that sits in the root of the project. It tells the agent:
*   Here is who you are.
...
We are leaving the era of static software. We are no longer building rigid machines that can only do what they are told. We are building flexible workshops filled with tools. The developer's job is to keep the tools sharp and the lights on. The agent's job is to build whatever the user needs, right in the moment.